        ...
        try:
            ...
            six.reraise(
                ...
                LocustError(
                    ...
                    % (type(self).__name__, self.task_set.__name__)
                ),
                sys.exc_info()[2],
            )
    ...
    def run(self, *args, **kwargs):
        ...
        try:
            ...
            if e.reschedule:
                ...
                six.reraise(
                    RescheduleTask, RescheduleTask(e.reschedule), sys.exc_info()[2]
                )

        while True:
            try:
                if (
                    self.locust.stop_timeout is not None
                    and time() - self._time_start > self.locust.stop_timeout
                ):
                    return
...
class TaskSequence(TaskSet):
    ...
    *max_wait* milliseconds. It will then schedule the `index + 1 % len(tasks)` task for execution and so on.
    ...
    def __init__(self, parent):
        ...
        self._index = 0
        self.tasks.sort(
            key=lambda t: t.locust_task_order if hasattr(t, "locust_task_order") else 1
        )
