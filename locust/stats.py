...
class RequestStats(object):
    ...
    def serialize_stats(self):
        return [
            ...
            for key in six.iterkeys(self.entries)
            if not (
...
class StatsEntry(object):
    ...
    def total_rps(self):
        ...
        return self.num_requests / max(
            self.stats.last_request_timestamp - self.stats.start_time, 1
        )
    ...
    def extend(self, other):
        ...
        self.min_response_time = (
            min(self.min_response_time or 0, other.min_response_time or 0)
    ...
    def __str__(self):
        try:
            fail_percent = (
                self.num_failures / float(self.num_requests + self.num_failures)
            ) * 100
        except ZeroDivisionError:
            fail_percent = 0
...
def on_slave_report(client_id, data):
    for stats_data in data["stats"]:
        ...
        if not request_key in global_stats.entries:
            global_stats.entries[request_key] = StatsEntry(
                global_stats, entry.name, entry.method
            )
